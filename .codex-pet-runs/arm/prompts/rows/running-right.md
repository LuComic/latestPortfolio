Create a single horizontal sprite strip for the Codex app digital pet `arm` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Short expressive dwarf-like performer in a vivid red suit, dark shirt accents, slicked-back hair, theatrical pointing hand, energetic mischievous grin; reinterpret as a friendly compact Codex digital pet, not a realistic likeness..

Style contract: Codex digital pet sprite style: pixel-art-adjacent low-resolution mascot sprite, compact chibi proportions, chunky whole-body silhouette, thick dark 1-2 px outline, visible stepped/pixel edges, limited palette, flat cel shading with at most one small highlight and one shadow step, simple readable face, tiny limbs, and no detail that disappears at 192x208. Avoid polished illustration, painterly rendering, anime key art, 3D render, vector app-icon polish, glossy lighting, soft gradients, realistic fur or material texture, anti-aliased high-detail edges, and complex tiny accessories. Additional user style notes: Codex digital pet sprite style: pixel-art-adjacent chibi proportions, thick dark outline, flat cel shading, limited red and black palette, stepped pixel edges, clean chroma-key background, no text, no scene curtains in the final sprite..
